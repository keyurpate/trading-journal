...
// COMPLETE INSTRUMENT MULTIPLIERS ($ per point) - Order matters: check specific first
const INSTRUMENT_MULTIPLIERS = [
    // Micros (check these first - more specific)
    ...
    { pattern: 'RB', multiplier: 42000, name: 'Gasoline' },
    { pattern: 'HO', multiplier: 42000, name: 'Heating Oil' },
    { pattern: '6E', multiplier: 12.5, name: 'Euro FX' },
    { pattern: '6B', multiplier: 6.25, name: 'British Pound' },
    { pattern: 'ZN', multiplier: 15.625, name: '10-Year T-Note' },
...
function calculatePnLFromTrade(trade) {
    ...
    console.log(`📊 P&L: ${trade.symbol} | ${trade.tradeType.toUpperCase()} | Entry: $${trade.entryPrice.toFixed(2)} → Exit: $${trade.exitPrice.toFixed(2)} | Qty: ${trade.quantity} | Mult: ${multiplier} | Diff: ${pointDiff.toFixed(2)} → P&L: $${pnl.toFixed(2)}`);
...
function setupEventListeners() {
    ...
    // Import backup
    const importBackupBtn = document.getElementById('importBackupButton');
        ...
        backupFileInput.addEventListener('change', function(e) {
            ...
            reader.onload = function(event) {
                try {
                    ...
                    if (confirm(`📂 Import ${importedTrades.length} trades from backup?\n\nThis will REPLACE all current trades!`)) {
                        trades = importedTrades;
                        
                        console.log('🔄 Recalculating P&L for imported trades...');
                    ...
                    alert('❌ Error importing backup. Make sure it\'s a valid JSON backup.');
                }
            };
